...
if __name__ == "__main__":
    ...
    DATADIR = os.getenv("PPDWIND_DATA_DIR")
    if DATADIR is None:
        raise OSError(
            "this script requires the $PPDWIND_DATA_DIR env variable to be set."
        )

    datafiles = sorted(glob(f"{DATADIR}/*dat"))
    medium = SSWMedium(datafiles[1])
    model = SSWDisk(medium=medium, mass_to_surface=3e-6)

    R0 = np.array([0, 0, 0, 1, np.pi / 4 + 0.1, np.pi / 2])
    record = RK4.integrate(model, R0, tstart=0, tstop=1_000, CFL=0.3, pbar=True)

    # row trajectory plots
    ...
    for colors, row in zip((None, "normal", "time"), axes):
        for normal, ax in zip("xyz", row):
            RecordPlot(record, ax, normal=normal, colors=colors)

    fig.savefig("/tmp/record_plot_cartesian.jpg", bbox_inches="tight")

    # poloidal trajectories
    fig, axes = plt.subplots(ncols=3, figsize=(25, 4))
    for colors, ax in zip((None, "normal", "time"), axes):
        RecordPlot(
            record, ax, geometry="cylindrical", colors=colors, normal="-phi", lw=4
        )
    fig.suptitle("poloidal trajectory plots")
    fig.savefig("/tmp/record_plot_poloidal.jpg", bbox_inches="tight")
